...
def compute_data_points_per_year(timeframe):
    if timeframe == '1m':
        ...
        data_points_per_year = 4 * 24 * 365
    elif timeframe == '30m':
        data_points_per_year = 2 * 24 * 365
    elif timeframe == '1h':
        data_points_per_year = 24 * 365
    elif timeframe == '4h':
        data_points_per_year = 6 * 365
    elif timeframe == '12h':
        data_points_per_year = 2 * 365
    elif timeframe == '1d':
...
def compute_eqw(price_ary, indx1, indx2):
    ...
    # Use forward percentage changes so benchmark returns are aligned with DRL returns
...
def sharpe_non_iid(rtns, bench=0, q=trading_days, p_critical=0.05, log=True):
    ...
    Returns:
        TYPE
    """
    if not isinstance(q, (np.int64, np.int32, int)):
