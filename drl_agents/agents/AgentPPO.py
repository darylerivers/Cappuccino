...
class AgentPPO(AgentBase):
    ...
    def update_net(self, buffer):
        ...
        with torch.no_grad():
            ...
            # Normalize advantages with NaN/zero-variance protection
            adv_std = buf_adv_v.std()
            if torch.isnan(adv_std) or adv_std < 1e-8:
                # Zero or near-zero variance - use unnormalized advantages
                buf_adv_v = buf_adv_v - buf_adv_v.mean()
            else:
                buf_adv_v = (buf_adv_v - buf_adv_v.mean()) / (adv_std + 1e-5)
        ...
        obj_critic = None
        obj_actor = None
        # Adjust batch_size if buffer is smaller (can happen with 1h timeframe)
        effective_batch_size = min(self.batch_size, buf_len)
        if effective_batch_size < self.batch_size:
            # Only warn once per agent instance to avoid terminal spam
            if not hasattr(self, '_batch_size_warning_printed'):
                print(f"Warning: Adjusting batch_size from {self.batch_size} to {effective_batch_size} (buf_len={buf_len})")
                self._batch_size_warning_printed = True
        for _ in range(int(1 + buf_len * self.repeat_times / effective_batch_size)):
